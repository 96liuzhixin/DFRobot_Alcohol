...
ADDRESS_0                 = 0x72           # iic slave Address
ADDRESS_1                 = 0x73
ADDRESS_2                 = 0x74
ADDRESS_3                 = 0x75
...
class DFRobot_Alcohol(object):
  ...
  def get_alcohol(self,reg):
    if self.__uart_iic == IIC_MODE:
      rslt = self.read_reg(reg ,2)
      return ((rslt[0] << 8) + rslt[1])
    elif self.__uart_iic == UART_MODE:
...
'''  
class DFRobot_Alcohol_IIC(DFRobot_Alcohol): 
  def __init__(self ,bus ,addr):
    self.__addr = addr;
    super(DFRobot_Alcohol_IIC, self).__init__(bus,0)
